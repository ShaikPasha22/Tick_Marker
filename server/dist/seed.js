...
async function seedDatabase() {
    ...
    if (existing) {
        await User_1.User.deleteOne({ _id: existing._id });
        await Habit_1.Habit.deleteMany({ userId: existing._id });
        await HabitCompletion_1.HabitCompletion.deleteMany({ userId: existing._id });
        await Goal_1.Goal.deleteMany({ userId: existing._id });
        await ExpenseCategory_1.ExpenseCategory.deleteMany({ userId: existing._id });
        await PaymentMethod_1.PaymentMethod.deleteMany({ userId: existing._id });
        await Expense_1.Expense.deleteMany({ userId: existing._id });
        await Income_1.Income.deleteMany({ userId: existing._id });
        await Budget_1.Budget.deleteMany({ userId: existing._id });
        await FinancialSettings_1.FinancialSettings.deleteMany({ userId: existing._id });
    ...
    const habits = await Habit_1.Habit.insertMany([
        {
            ...
            status: 'active',
            startDate: (0, date_fns_1.subDays)(today, 60),
            order: 0,
        },
        {
            userId,
        ...
        },
        {
            ...
            status: 'active',
            startDate: (0, date_fns_1.subDays)(today, 45),
            order: 2,
        },
        {
            userId,
        ...
        },
        {
            ...
            schedule: { frequency: 'daily' },
            status: 'active',
            startDate: (0, date_fns_1.subDays)(today, 20),
            order: 6,
        },
    ]);
    console.log(`📋 Created ${habits.length} habits`);
    // Generate completions for the past 60 days
    const days = (0, date_fns_1.eachDayOfInterval)({ start: (0, date_fns_1.subDays)(today, 59), end: (0, date_fns_1.subDays)(today, 1) });
    const completions = [];
    for (const day of days) {
        ...
        for (const habit of habits) {
            ...
            if (!isScheduled)
                continue;
            if (utcDay < habit.startDate)
                continue;
            // Simulate realistic completion rates
            const completionChance = habit.name === 'Morning Meditation' ? 0.88 :
            ...
            if (rand < completionChance) {
                ...
                status = 'partial';
                if (habit.type === 'duration')
                    value = Math.round(habit.target * 0.6);
                else if (habit.type === 'quantity')
                    value = parseFloat((habit.target * 0.6).toFixed(1));
                else if (habit.type === 'count')
                    value = Math.round(habit.target * 0.5);
            }
    ...
    await HabitCompletion_1.HabitCompletion.insertMany(completions, { ordered: false }).catch(() => { });
    console.log(`✅ Seeded ${completions.length} completion records`);
    // Create goals
    await Goal_1.Goal.insertMany([
        {
            userId,
    ...
    const expenseCategories = await ExpenseCategory_1.ExpenseCategory.insertMany(ExpenseCategory_1.DEFAULT_EXPENSE_CATEGORIES.map((cat) => ({ ...cat, userId, type: 'expense', isDefault: true })));
    // Create income categories
    const incomeCategories = await ExpenseCategory_1.ExpenseCategory.insertMany(ExpenseCategory_1.DEFAULT_INCOME_CATEGORIES.map((cat) => ({ ...cat, userId, type: 'income', isDefault: true })));
    // Create payment methods
    const paymentMethods = await PaymentMethod_1.PaymentMethod.insertMany(PaymentMethod_1.DEFAULT_PAYMENT_METHODS.map((pm) => ({ ...pm, userId })));
    // Financial settings
    await FinancialSettings_1.FinancialSettings.create({
        userId,
    ...
    await Income_1.Income.insertMany(incomeRecords);
    ...
    const expDays = (0, date_fns_1.eachDayOfInterval)({ start: (0, date_fns_1.subDays)(today, 59), end: today });
    for (const day of expDays) {
        const utcDay = new Date(day);
        utcDay.setUTCHours(0, 0, 0, 0);
        for (const ed of expenseData) {
            if (Math.random() > ed.frequency)
                continue;
    ...
    await Expense_1.Expense.insertMany(expenseRecords, { ordered: false }).catch(() => { });
